...
export default function MoreScreen() {
  ...
  const handleScreenshotLoad = async () => {
    Alert.alert(
      '📸 Mode Screenshot',
      'Charger des données de démonstration pour screenshots ?\n\nCela va créer un profil complet avec 3 mois de données (pesées, entraînements, mensurations, etc.).',
      [
        { text: 'Annuler', style: 'cancel' },
        {
          text: 'Charger',
          onPress: async () => {
            try {
              ...
              if (result.success) {
                ...
                Alert.alert('❌ Erreur', result.error || 'Impossible de charger les données');
              }
            } catch (e) {
