...
export default function PlanningScreen() {
  ...
  const styles = useMemo(() => createStyles(SCREEN_WIDTH, IS_SMALL_SCREEN), [SCREEN_WIDTH, IS_SMALL_SCREEN]);
  ...
  // Charger après la fin de l'animation de navigation (évite le jank)
  useEffect(() => {
  ...
  // Load events per region — only what's needed, max 500 par vue
  const loadCatalogForRegion = useCallback(async (region: CatalogLocationFilter) => {
    setCatalogLoading(true);
    try {
      let filters: Parameters<typeof getFilteredEvents>[0] = { upcomingOnly: true, limit: 500 };
      if (region === 'france') {
        filters.country = 'France';
      } else if (region === 'europe') {
        // Charger tous les events à venir et filtrer côté client pour l'Europe
        filters.limit = 1100;
      }
  ...
  // Charger les événements seulement quand l'onglet "competitions" est actif
  useEffect(() => {
    if (viewMode !== 'competitions') return;
    loadCatalogForRegion(catalogLocationFilter);
  }, [catalogLocationFilter, viewMode, loadCatalogForRegion]);
...
const createStyles = (SCREEN_WIDTH: number, IS_SMALL_SCREEN: boolean) => StyleSheet.create({
  container: {
    flex: 1,
  },
  scrollView: {
