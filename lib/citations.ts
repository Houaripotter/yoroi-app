...
const DISCIPLINE_QUOTES: Citation[] = [
  ...
  { text: "Tu ne regrettes jamais un entraînement. Tu regrettes celui que tu as rate.", category: 'discipline' },
...
const PERSEVERANCE_QUOTES: Citation[] = [
  ...
  { text: "La seule mauvaise séance est celle qui n'a pas eu lieu.", category: 'perseverance' },
  { text: "Tu es plus fort que tu ne le penses.", category: 'perseverance' },
  { text: "Celui qui sue plus a l'entraînement saigne moins au combat.", category: 'perseverance' },
...
export const getDailyQuote = async (): Promise<Citation> => {
  try {
    ...
    const cacheKey = LAST_CITATION_DATE_KEY;
    const citationKey = LAST_CITATION_KEY;
