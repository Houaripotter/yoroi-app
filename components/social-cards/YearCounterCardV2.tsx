            ...
            <View style={{ position: 'absolute', top: 8, left: 0, right: 0, alignItems: 'center', zIndex: 10 }}>
              {keepPhotoClear ? (
                <View style={{ backgroundColor: 'rgba(0,0,0,0.85)', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 8 }}>
                  <Text style={[styles.dateText, { color: GOLD_COLOR }]}>{yearLabel}</Text>
                </View>
              ) : (
          ...
          <View style={styles.photoBottomInfo}>
            {keepPhotoClear ? (
              <View style={{ backgroundColor: 'rgba(0,0,0,0.85)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, alignSelf: 'flex-start' }}>
                <Text style={[styles.mainTitle, { color: '#FFF' }]}>RÉCAP ANNUEL</Text>
                <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 4, marginTop: 4 }}>
                  <Text style={[styles.bigNumber, { color: GOLD_COLOR }]}>{stats.totalDays}</Text>
                  <Text style={[styles.bigLabel, { color: '#FFF' }]}>/365 JOURS</Text>
                </View>
              </View>
            ) : (
              <>
        ...
        <View style={styles.statsSection}>
          ...
          <View style={{ marginBottom: 8 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
              <Text style={[styles.sectionLabel, { color: GOLD_COLOR }]}>OBJECTIF {yearlyGoal} JOURS</Text>
          ...
          <View style={[styles.statsRow, { backgroundColor: isWhite ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.08)', borderColor }]}>
            <View style={{ flex: 1, alignItems: 'center' }}>
              <Text style={{ color: GOLD_COLOR, fontSize: 16, fontWeight: '900' }}>{stats.bestStreak}</Text>
              <Text style={{ color: subTxt, fontSize: 8, fontWeight: '800' }}>BEST STREAK</Text>
            </View>
            <View style={{ width: 1, height: 30, backgroundColor: borderColor }} />
            <View style={{ flex: 1, alignItems: 'center' }}>
              <Text style={{ color: GOLD_COLOR, fontSize: 16, fontWeight: '900' }}>{stats.busiestMonth?.month.substring(0, 3) || '-'}</Text>
              <Text style={{ color: subTxt, fontSize: 8, fontWeight: '800' }}>TOP MOIS</Text>
            </View>
            <View style={{ width: 1, height: 30, backgroundColor: borderColor }} />
            <View style={{ flex: 1, alignItems: 'center' }}>
              <Text style={{ color: GOLD_COLOR, fontSize: 16, fontWeight: '900' }}>{Math.round(stats.percentage)}%</Text>
              <Text style={{ color: subTxt, fontSize: 8, fontWeight: '800' }}>DE L'ANNÉE</Text>
            </View>
          </View>
        </View>
