...
export function AnimatedTabBar({ state, descriptors, navigation }: TabBarProps) {
  ...
  // Couleurs du bouton + : gradient des 2 couleurs du combo
  const isLightMode = !isDark;

  // Bouton + : blanc sur fond accent (light) / accent sur fond noir (dark)
  ...
  const plusIconColor = isLightMode ? colors.accent : colors.textOnAccent;

  // Style special pour le bouton selon le mode
  let plusButtonStyle: any = isLightMode ? {
    // Mode clair: bouton blanc avec bordure accent
    shadowColor: colors.accentDark || colors.accent,
  ...
  // Couleurs - accent bg en light, noir en dark
  const tabBarBg = isDark ? '#0D0D0F' : colors.accent;
  const tabBarInactiveColor = isDark ? 'rgba(255,255,255,0.45)' : 'rgba(255,255,255,0.55)';
  const tabBarActiveColor = isDark ? colors.accent : '#FFFFFF';
  const tabBarCompanionColor = isDark ? colors.companion : '#FFFFFF';
        ...
        <Animated.View
          style={[
            styles.pulseRing,
            {
              ...
              backgroundColor: isLightMode ? 'rgba(255,255,255,0.3)' : `${colors.accent}15`,
              borderColor: isLightMode ? 'rgba(255,255,255,0.5)' : `${colors.accent}30`,
            },
          ]}
        />
        ...
        <View style={styles.tabsContainer}>
          {state.routes.map((route, index) => {
            ...
            return (
              ...
              >
                {/* Pastille companion derrière l'onglet actif */}
                {isFocused && (
                  <View style={[
                    styles.activeTabPill,
                    { backgroundColor: tabBarCompanionColor + '25' },
                  ]} />
                )}
                <View style={[
                  styles.iconWrapper,
                ...
                <Animated.Text
                  style={[
                    styles.tabLabel,
                    {
                      color: isFocused ? tabBarCompanionColor : tabBarInactiveColor,
